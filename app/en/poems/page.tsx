...
export default function PoemsPage() {
  const posts = getPostsByCategory('en', 'poems')
  return (
    <div className="max-w-layout mx-auto px-6 py-8 md:py-16">
      <header className="mb-4 pb-4">
        <h1 className="font-serif text-4xl md:text-5xl font-bold tracking-tight">Poems</h1>
      ...
      {posts.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">{posts.map(post => <PostCard key={post.slug} {...post} locale="en" href={`/en/poems/${post.slug}`} />)}</div>
      )}
    </div>
  )
}
