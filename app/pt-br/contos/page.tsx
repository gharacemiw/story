...
export const metadata: Metadata = { title: 'Contos' }

export default function ContosPage() {
  const posts = getPostsByCategory('pt-br', 'contos')
  return (
    <div className="max-w-layout mx-auto px-6 py-16">
      <header className="mb-4 pb-4">
        <h1 className="font-serif text-4xl md:text-5xl font-bold tracking-tight">Contos</h1>
      ...
      {posts.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">{posts.map(post => <PostCard key={post.slug} {...post} locale="pt-br" href={`/pt-br/contos/${post.slug}`} />)}</div>
      )}
    </div>
  )
}
