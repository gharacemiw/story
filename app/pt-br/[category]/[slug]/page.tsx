...
const CATEGORIES = ['contos', 'poemas', 'alucinacoes'] as const
const categoryLabels: Record<string, string> = { contos: 'Contos', poemas: 'Poemas', alucinacoes: 'Alucinações' }
    ...
    <div className="max-w-reading mx-auto px-6 py-16 md:py-24">
      ...
      <nav className="mt-16 pt-8 flex justify-between gap-8">
        <div className="flex-1">
          {prev ? (
            <Link href={`/pt-br/${category}/${prev.slug}`} className="group flex flex-col gap-1">
              <span className="font-sans text-xs text-warm-muted dark:text-dark-muted uppercase tracking-wider">Anterior</span>
            ...
            <Link href="/pt-br" className="group flex flex-col gap-1">
              <span className="font-sans text-xs text-warm-muted dark:text-dark-muted uppercase tracking-wider">Você chegou ao fim</span>
              <span className="font-sans text-base font-medium group-hover:text-warm-accent dark:group-hover:text-dark-accent transition-colors">← Voltar ao início</span>
            </Link>
          )}
        </div>
        <div className="flex-1 text-right">
          {next && (
            <Link href={`/pt-br/${category}/${next.slug}`} className="group flex flex-col gap-1 items-end">
              <span className="font-sans text-xs text-warm-muted dark:text-dark-muted uppercase tracking-wider">Próximo</span>
